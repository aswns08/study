...
$(document).ready(function() {
	$("#comment_write").click(function() {
		$.ajax({
			type: "POST",
			url: "/daejeon_bbs_ibatis/commentWrite.comment",
			async: true,
			dataType: "json",
			data: {
...
function getComment(commPageNum, event) {
	...
	$.ajax({
		type : "POST",
		url : "/daejeon_bbs_ibatis/commentRead.comment",
		async : true,
		dataType : "json",
		data : {
			article_Num : $("#article_Num").val(),
...
<form action="/daejeon_bbs_ibatis/replyForm.daejeon" method="post">
	...
	<table border="1" width="500" align="center">
		...
		<tr>
			...
			<td colspan="2"><a href="/daejeon_bbs_ibatis/download.daejeon?fname=${article.fname}">${article.fname}</a></td>
		</tr>
		<tr>
		...
		<tr>
	      <c:if test="${id !=null}">
	    	  ...
	    	  <input type="button" value="수정하기" onclick="document.location.href='/daejeon_bbs_ibatis/updateForm.daejeon?article_Num=${article.article_Num}&pageNum=${pageNum}'">
	    	  <input type="button" value="삭제하기" onclick="document.location.href='/daejeon_bbs_ibatis/delete.daejeon?article_Num=${article.article_Num}&pageNum=${pageNum}'">
	    	  </c:if>
	    	  <c:if test="${id !=article.id}">
	    	  <input type="button" value="수정하기" disabled="disabled">
	    	  <input type="button" value="삭제하기" disabled="disabled">
	    	  </c:if>
	    	  <input type="button" value="목록으로" onclick="document.location.href='/daejeon_bbs_ibatis/list.daejeon?pageNum=${pageNum}'">
	    	  </td>
	      </c:if>
